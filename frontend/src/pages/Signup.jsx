...
export default function Signup(){
  ...
  const onSubmit = async (e) => {
    ...
    try { const { data } = await api.post('/api/auth/register', { name, email, password, role:'employee' }); login(data.token); nav('/'); }
  ...
  return (
    <div className="page">
      <div className="max-w-md mx-auto card">
        ...
        <p className="text-sm text-slate-600 mt-3">Already have an account? <Link className="text-blue-600" to="/login">Login</Link></p>
      </div>
    </div>
  )
}
